...
export function DashboardPage() {
  ...
  const cards = [
    data?.clientes && { label: 'Clientes', value: data.clientes.total, icon: Users, iconColor: 'text-sky-500', iconBg: 'bg-sky-50' },
    data?.productos && { label: 'Productos', value: data.productos.total, icon: Package, iconColor: 'text-emerald-500', iconBg: 'bg-emerald-50' },
    data?.facturas && { label: 'Facturas', value: data.facturas.total, icon: FileText, iconColor: 'text-amber-500', iconBg: 'bg-amber-50' },
    data?.empleados && { label: 'Empleados', value: data.empleados.total, icon: UserCog, iconColor: 'text-violet-500', iconBg: 'bg-violet-50' },
            ...
            return (
              <div key={card.label} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`flex items-center justify-center w-12 h-12 shrink-0 rounded-xl ${card.iconBg}`}>
